...
function VerifyCodeAdmin() {
  ...
  const verify_code = async (e) => {
    e.preventDefault();
    let url_hit = `${API_DUMMY}/api/admin/validasi-code`;
    try {
      ...
      if (response.status === 200) {
        window.location.href = "/reset-password-admin/" + code;
        Swal.fire({
          icon: "success",
  ...
  const send_email = async (e) => {
    e.preventDefault();
    let url_hit = `${API_DUMMY}/api/admin/forgot_password`;
    try {
      const response = await axios.post(url_hit, {
        email,
      });
